...
if(isset($_POST['submit1'])){
		...
		$from= $_POST['date_from1'];
		$to= $_POST['date_to1'];
		
		$query = "SELECT * FROM feedback where date between '$from' and '$to'"; 
		$result = mysqli_query($con,$query);
	
	?>
		<h4><center>Report Generated As on <?php echo date("Y-m-d") ?><center></h4>
	...
	<?php
		 }elseif(isset($_POST['submit2']))
		 {
			...
			$from= $_POST['date_from2'];
			$to= $_POST['date_to2'];
			
			$query = "SELECT * FROM propertydetails where date between '$from' and '$to'"; 
			$result = mysqli_query($con,$query);
